...
export class UserGuard{
  ...
  canActivate(
    ...
    if(this._accountService.user){
      if( this._accountService.user.roleId==Roles.moderator || Roles.admin || Roles.user){
        return true;
      }else{
    ...
    if(this._storageService.userPermission){
      if(this._storageService.userPermissionNumber==Roles.moderator || Roles.admin || Roles.admin){
        return true;
      }else{
        this._toastSerivce.error('You dont have enough permissions to proceed further',this._accountService.currentLoggedUser.email)
        this._router.navigate(['/home']);
